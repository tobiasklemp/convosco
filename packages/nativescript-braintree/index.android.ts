...
class DataCollector {
	getDeviceData(fragment: com.braintreepayments.api.BraintreeFragment, cb: (data) => void) {
		com.braintreepayments.api.DataCollector.collectDeviceData(
			fragment,
			new com.braintreepayments.api.interfaces.BraintreeResponseListener<string>({
...
export class BraintreePaymentHandler {
	...
	constructor(private fragment: com.braintreepayments.api.BraintreeFragment, private cb: (response: BraintreeListenerResponse) => void, private request?: any) {
		let that = this;
	...
	startLocalPayment(): void {
		this.addListeners();
		let that = this;
		com.braintreepayments.api.LocalPayment.startPayment(
			this.fragment,
			this.request,
			new com.braintreepayments.api.interfaces.BraintreeResponseListener<com.braintreepayments.api.models.LocalPaymentRequest>({
				onResponse(request) {
					that.fragment.addListener(
						new com.braintreepayments.api.interfaces.PaymentMethodNonceCreatedListener({
							onPaymentMethodNonceCreated(nonce) {
								that.cb({ nonce: new PaymentMethodNonce(nonce) });
							},
						})
	...
	startGooglePayPayment(): void {
		this.addListeners();
		let that = this;
		com.braintreepayments.api.GooglePayment.isReadyToPay(
			this.fragment,
			new com.braintreepayments.api.interfaces.BraintreeResponseListener({
				onResponse(res) {
					if (res) {
...
export class Braintree extends BraintreeBase {
	...
	public startPaypalCheckoutPayment(options: BrainTreeOptions): Promise<any> {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let paypal = new BraintreePaymentHandler(
					fragment,
					(response: { nonce?: IPayPalAccountNonce; cancelled?: number; error?: any }) => {
						if (response.error) {
	...
	public startLocalPayment(options: BrainTreeOptions): Promise<any> {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let local = new BraintreePaymentHandler(
					fragment,
					(response: { nonce?: IPaymentMethodNonce; cancelled?: number; error?: any }) => {
						if (response.error) {
							reject(response.error);
						} else if (response.cancelled) {
							reject(response.cancelled);
	...
	public startPaypalVaultPayment(options: BrainTreeOptions): Promise<IPayPalAccountNonce> {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let paypal;
				paypal = new BraintreePaymentHandler(
					fragment,
					(response: { nonce?: IPayPalAccountNonce; cancelled?: number; error?: any }) => {
						if (response.error) {
							reject(response.error);
	...
	startGooglePayPayment(options: BrainTreeOptions): Promise<IPaymentMethodNonce> {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let googlePaymentRequest = new com.braintreepayments.api.models.GooglePaymentRequest().transactionInfo(com.google.android.gms.wallet.TransactionInfo.newBuilder().setTotalPrice(options.amount).setTotalPriceStatus(com.google.android.gms.wallet.WalletConstants.TOTAL_PRICE_STATUS_FINAL).setCurrencyCode(options.currencyCode).build()).billingAddressRequired(true);

				let googlePay = new BraintreePaymentHandler(
					fragment,
					(response) => {
						if (response.error) {
							reject(response.error);
	...
	public startPayment(token: any, options: BrainTreeOptions): Promise<{ nonce: string; deviceData: string }> {
		return new Promise((resolve, reject) => {
			...
			if (options.requestThreeDSecureVerification && options.amount) {
				...
				dropInRequest.requestThreeDSecureVerification(true).threeDSecureRequest(threeDSecureRequest);
			}
	...
	private enableGooglePay(dropInRequest: com.braintreepayments.api.dropin.DropInRequest, options: BrainTreeOptions): void {
		...
		let googlePaymentRequest = new GooglePaymentRequest().transactionInfo(TransactionInfo.newBuilder().setTotalPrice(options.amount).setTotalPriceStatus(WalletConstants.TOTAL_PRICE_STATUS_FINAL).setCurrencyCode(options.currencyCode).build()).billingAddressRequired(true);
